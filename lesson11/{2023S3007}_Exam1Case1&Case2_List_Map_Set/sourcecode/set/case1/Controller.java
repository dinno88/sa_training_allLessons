package edu.satraining.lesson11.set.exam1.case1.controller;

import edu.satraining.lesson11.set.exam1.case1.model.Book;
import edu.satraining.lesson11.set.exam1.case1.model.BookStore;
...
public class Controller {
    ...
    public void sortingData() {
        Set<Book> sortedBooks = quickSortByPriceDescending(new HashSet<>(books));
        books.clear();
        books.addAll(sortedBooks);
    }

    private Set<Book> quickSortByPriceDescending(Set<Book> set) {
        if (set.size() <= 1) {
            return set;
        }

        float pivot = set.iterator().next().getPrice();
        Set<Book> less = new HashSet<>();
        Set<Book> pivotSet = new HashSet<>();
        Set<Book> greater = new HashSet<>();
        ...
        Set<Book> sortedSet = new HashSet<>();
        sortedSet.addAll(quickSortByPriceDescending(greater));
        sortedSet.addAll(pivotSet);
        sortedSet.addAll(quickSortByPriceDescending(less));
